...
test('contract fixtures preserve cross-route invariants used by the frontend', () => {
  ...
  assert.equal(signalFixture.decision_contract.evidence.status, 'BLOCKED');
  assert.equal(signalFixture.decision_contract.action_authorized, false);
  assert.equal(signalFixture.edge_quality?.validated_edge, false);
  assert.equal(planFixture.decision_contract.action_authorized, planFixture.action_now.action_authorized);
  assert.equal(planFixture.decision_contract.actionability_state, planFixture.actionability_state);
  assert.equal(planFixture.edge_quality.validated_edge, planFixture.edge_evidence_gate?.pass);
  assert.equal(planFixture.trader_playbook.authorization, 'AUTHORIZED');
  assert.equal(planFixture.opportunity_ref?.cta_enabled, true);
  ...
  assert.equal(categoryDetailFixture.indicators[0]?.id, 'ism_manufacturing');
  assert.equal(categoryDetailFixture.indicators[0]?.canonical_id, 'manufacturing_payrolls');
  assert.equal(categoryDetailFixture.indicators[0]?.freshness.sla_class, 'monthly');
  assert.equal(categoryDetailFixture.indicators[0]?.freshness.status, 'fresh');
