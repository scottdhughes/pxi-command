...
function getEmbeddingVector(embedding: unknown): number[] {
  return (embedding as { data: number[][] }).data[0];
}
...
function extractLSTMFeatures(
  data:
    | { pxi_score: number; pxi_delta_7d: number | null; categories: Record<string, number>; vix?: number }
    | { score: number; delta_7d: number | null; categories: Record<string, number>; vix?: number },
  norm: Record<string, { mean: number; std: number }>,
  featureNames: string[]
): number[] {
  const pxiScore = 'pxi_score' in data ? data.pxi_score : data.score;
  const pxiDelta7d = 'pxi_delta_7d' in data ? data.pxi_delta_7d : data.delta_7d;
  const features: Record<string, number> = {
    pxi_score: pxiScore,
    pxi_delta_7d: pxiDelta7d ?? 0,
...
async function handleScheduled(env: Env): Promise<void> {
  ...
  if (result) {
    ...
    try {
      ...
      const todayEmbeddingVector = getEmbeddingVector(todayEmbedding);

      const similar = await env.VECTORIZE.query(todayEmbeddingVector, {
        topK: 50,
        returnMetadata: 'all',
      });
...
export default {
  ...
  <text x="1140" y="612" text-anchor="end" font-family="'SF Mono', 'Menlo', monospace" font-size="10" fill="#949ba5" opacity="0.3" letter-spacing="2">PXICOMMAND.COM</text>
      ...
      const marketDomainDeps = {
        INDICATOR_FREQUENCY_HINTS,
        addCalendarDays,
        applyCrossHorizonActionabilityOverride,
        asIsoDate,
        asIsoDateTime,
        buildBriefFallbackSnapshot,
        buildBriefSnapshot,
        buildConsistencySnapshot,
        buildConvictionCalibrationSnapshot,
        buildCurrentBucketRiskBands,
        buildDecisionImpactMarketStats,
        buildDecisionImpactThemeStats,
        buildDecisionStack,
        buildEdgeDiagnosticsReport,
        buildEdgeQualityCalibrationFromSnapshot,
        buildEdgeQualityCalibrationSnapshot,
        buildHistoricalOpportunitySnapshot,
        buildInvalidationRules,
        buildOpportunityFallbackSnapshot,
        buildOpportunityLedgerProjection,
        buildOpportunitySnapshot,
        buildPolicyStateSnapshot,
        buildPlanFallbackPayload,
        buildTraderPlaybookSnapshot,
        calculatePXISignal,
        canSendEmail,
        clamp,
        computeCalibrationDiagnostics,
        computeEdgeQualitySnapshot,
        computeFreshnessDiagnostics,
        computeFreshnessStatus,
        computeFreshnessSloWindow,
        computeNextExpectedRefresh: computeNextExpectedRefreshModule,
        computeUtilityFunnelSummary,
        ...
        fetchCalibrationSnapshotAtOrBefore,
        fetchDecisionImpactSnapshotAtOrBefore,
        fetchLatestCalibrationSnapshot,
        fetchLatestConsistencyCheck,
        fetchPredictionEvaluationSampleSize,
        freshnessPenaltyCount,
        generateToken,
        getAlertsSigningSecret,
        hashToken,
        insertMarketEvents,
        insertOpportunityItemLedgerRow,
        insertOpportunityLedgerRow,
        insertUtilityEvent,
        ...
        normalizeUtilityActionabilityState,
        normalizeUtilityEventType,
        normalizeUtilityRoute,
        normalizeUtilitySessionId,
        parseBackfillDateRange,
        parseBackfillLimit,
        parseIsoDate,
        parseJsonBody,
        priceOnOrAfterSeries,
        projectOpportunityFeed,
        recordMarketRefreshRunFinish,
        recordMarketRefreshRunStart,
        ...
        stableHash,
        storeBriefSnapshot,
        storeCalibrationSnapshot,
        storeConsistencyCheck,
        storeDecisionImpactSnapshot,
        storeOpportunitySnapshot,
        summarizeCrossHorizonCoherence,
        toNumber,
        validateEmail,
      };

      const marketCoreResponse = await tryHandleMarketCoreRoute(marketRouteContext, marketDomainDeps);
      if (marketCoreResponse) {
        return marketCoreResponse;
      }

      const marketProductsResponse = await tryHandleMarketProductsRoute(marketRouteContext, marketDomainDeps);
      if (marketProductsResponse) {
        return marketProductsResponse;
      }

      const marketOpsResponse = await tryHandleMarketOpsRoute(marketRouteContext, marketDomainDeps);
      if (marketOpsResponse) {
        return marketOpsResponse;
      }
      ...
      if (url.pathname === '/api/similar' && method === 'GET') {
        try {
          ...
          const embeddingVector = embedding ? getEmbeddingVector(embedding) : null;
        ...
        try {
          ...
          if (indicators.results && indicators.results.length >= 5) {
            ...
            try {
              ...
              const similar = await env.VECTORIZE.query(embeddingVector, {
                topK: 50,
                returnMetadata: 'all',
              });
